...
## 12. Worked example — full YAML (extended variant)

The extended example diagram is fully described by the following inputs. Every coordinate in the rendered SVG is derivable from this block via §2 + §3 + §4. This is the canonical proof that the parametric contract works end-to-end. The rendered file is **not shipped with this skill** — regenerate it from this YAML if the extended variant is needed.
...
- *Extended pattern (not shipped as a file):* exercises §4 color override — Build app in slate-blue (data quality), Train enumerators in rust-red (governance), Publish results in olive-green (data products). Focal accent on Pilot test step + node unchanged. Regenerate from §12's YAML if this variant is needed.
