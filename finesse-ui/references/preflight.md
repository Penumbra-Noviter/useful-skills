...
1. **Static (always):** run the detector from the skill root (the directory containing `scripts/`) — it greps for a real engine and the reduced-motion fallback, and fails on "claimed-not-shown":
   ```bash
