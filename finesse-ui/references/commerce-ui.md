...
The **commerce** register: a hybrid of brand and product. A product detail page (PDP) sells the vibe of *one* item and can reach for soul (§4 in SKILL.md); a listing/category page (PLP), cart, and checkout serve a task — compare, filter, complete a purchase — and behave like the **product** register (density up, spectacle down). Route each page on its own job; don't apply one register to the whole flow. See SKILL.md §0.A for the PDP-vs-PLP call.
...
- [ ] Still passes the substrate + cheapness blacklist (SKILL.md §5, §8).
