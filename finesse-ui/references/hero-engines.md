...
### Canonical GSAP Skeletons (React/Next.js)

These are the three essential scroll-driven patterns — **Sticky Stack**, **Horizontal Pan**, and **Scroll-Reveal Stagger** — as complete, self-contained React components. Each is a canonical implementation with proper cleanup, reduced-motion support, and the critical `start: "top top"` / `pin: true` / `scrub` configuration that commonly fails when hand-coded from scratch.

**Sticky-Stack** — cards pin at the viewport top and physically stack on scroll. Every card except the last is pinned; the previous card scales/opacity-transforms as the next card arrives.
...
**Horizontal-Pan** — vertical scroll drives a horizontal slide across a wide track. The wrapper is pinned; the inner track slides horizontally.
...
**Critical points:** `start: "top top"`, `pin: true`, `end: "+=${distance}"` (scroll length = horizontal travel needed), `scrub: 1`, `invalidateOnRefresh: true`. The wrapper is pinned, the inner track slides horizontally as the user scrolls vertically.

**Scroll-Reveal Stagger** — lighter alternative for "items appear as they enter viewport." No pinning, no scrub. Use Motion's `whileInView` over GSAP for this — lighter, no plugin needed.
...
These patterns are banned for all engines, not just GSAP. Each has a specific technical reason:

- **`window.addEventListener("scroll", handler)`** driving animation on every scroll frame. → Use Motion's `useScroll()`, GSAP's `ScrollTrigger`, IntersectionObserver, or CSS `scroll-driven animations` (`animation-timeline: view()`). The raw scroll event listener runs on every frame without batching, causing continuous reflows and killing mobile performance.
- **Custom scroll progress calculations using `window.scrollY`** in React state. → Same re-render-on-every-frame problem. Use motion values (`useMotionValue` + `useTransform`) instead.
- **`requestAnimationFrame` loops touching React state.** → Use motion values or GSAP context; never `setState` inside a `rAF` — it triggers a React tree re-render on every frame.
- **`useState` to track continuous pointer-driven values** (mouse position, scroll progress, magnetic hover). → Use Motion's `useMotionValue` / `useTransform` / `useScroll` outside the React render cycle. `useState` collapses on mobile under rapid pointer events.
- **Layout Transitions:** Use Motion's `layout` and `layoutId` for visible state changes (re-ordering lists, expanding modals, shared elements). Do not wrap static content in `layout` props "for safety" — it costs measurement work.
- **No `prefers-reduced-motion` fallback.** → Mandatory for every animated page. Freeze to a still frame, not a skipped animation (the element must still render in its final state).
- **`power1.out` / linear easing on reveals.** Too fast = cheap. → `power3.out` / `cubic-bezier(0.16, 1, 0.3, 1)` / spring physics.
- **Missing `ScrollTrigger.getAll().forEach(t => t.kill())` on unmount in SPAs.** → Causes phantom scroll triggers that fire on other pages.
- **Missing `renderer.dispose()` on Three.js WebGLRenderer.** → Leaks GPU memory.
