...
The patterns below are a vocabulary, not a library. The agent should *know* these names to communicate about them, design with them in mind, and reach for them when the design read calls for them. Each pattern has a canonical implementation path in `references/hero-engines.md` or `references/3d-effects.md` when applicable.

### Hero Paradigms
| Pattern | Description | Engine |
|---------|-------------|--------|
| **Asymmetric Split Hero** | Text on one side, asset on the other, generous white space | D/E |
| **Editorial Manifesto Hero** | Large type, no asset, almost-poster | E |
| **Video / Media Mask Hero** | Type cut out as mask over video background | E |
| **Kinetic-Type Hero** | Animated typography as the primary visual | D/E |
| **Curtain-Reveal Hero** | Hero parts on scroll like a curtain | D |
| **Scroll-Pinned Hero** | Hero stays pinned while content scrolls behind | D |

### Navigation & Menus
| Pattern | Description | Notes |
|---------|-------------|-------|
| **Mac OS Dock Magnification** | Edge nav, icons scale fluidly on hover | CSS `transform: scale()` on hover |
| **Magnetic Button** | Pulls toward cursor | hero-engines.md §Extended |
| **Gooey Menu** | Sub-items detach like viscous liquid | SVG filter, rare |
| **Dynamic Island** | Morphing pill for status / alerts | CSS transitions |
| **Contextual Radial Menu** | Circular menu expanding at click point | JS-driven, rare |
| **Floating Speed Dial** | FAB springing into curved secondary actions | Motion stagger |
| **Mega Menu Reveal** | Full-screen dropdown, stagger-fade content | GSAP stagger |

### Layout & Grids
| Pattern | Description | Notes |
|---------|-------------|-------|
| **Bento Grid** | Asymmetric tile grouping (Apple Control Center) | CSS Grid, `grid-flow-dense` |
| **Masonry Layout** | Staggered grid, no fixed row height | CSS columns or Masonry |
| **Chroma Grid** | Borders / tiles with subtle animating gradients | CSS animation |
| **Split-Screen Scroll** | Two halves sliding in opposite directions | GSAP/ScrollTrigger |
| **Sticky-Stack Sections** | Sections that pin and stack on scroll | hero-engines.md §GSAP skeletons |

### Cards & Containers
| Pattern | Description | Notes |
|---------|-------------|-------|
| **Parallax Tilt Card** | 3D tilt tracking mouse coordinates | 3d-effects.md §1 |
| **Spotlight Border Card** | Borders illuminate under cursor | CSS radial-gradient mask |
| **Glassmorphism Panel** | Frosted glass with inner refraction | `backdrop-filter`, solid fallback |
| **Holographic Foil Card** | Iridescent rainbow shift on hover | CSS `hue-rotate` / gradient |
| **Tinder Swipe Stack** | Physical card stack, swipe-away | Pointer events + transform |
| **Morphing Modal** | Button expands into its own dialog | View Transitions / Motion layout |

### Scroll Animations
| Pattern | Description | Engine |
|---------|-------------|--------|
| **Sticky Scroll Stack** | Cards stick and physically stack | D (canonical skeleton) |
| **Horizontal Scroll Hijack** | Vertical scroll → horizontal pan | D (canonical skeleton) |
| **Locomotive / Sequence Scroll** | Video / 3D sequence tied to scroll position | D or custom |
| **Zoom Parallax** | Central background image zooming on scroll | D |
| **Scroll Progress Path** | SVG line drawing along scroll | D |
| **Liquid Swipe Transition** | Page transition like viscous liquid | CSS/SVG filter |

### Galleries & Media
| Pattern | Description | Engine |
|---------|-------------|--------|
| **Dome Gallery** | 3D panoramic gallery | A (Three.js) |
| **Coverflow Carousel** | 3D carousel with angled edges | E (CSS 3D) |
| **Drag-to-Pan Grid** | Boundless draggable canvas | Pointer events |
| **Accordion Image Slider** | Narrow strips expanding on hover | CSS `flex` grow |
| **Hover Image Trail** | Mouse leaves popping image trail | Canvas |
| **Glitch Effect Image** | RGB-channel shift on hover | CSS `mix-blend-mode` |

### Typography & Text
| Pattern | Description | Engine |
|---------|-------------|--------|
| **Kinetic Marquee** | Endless text bands reversing on scroll | D |
| **Text Mask Reveal** | Massive type as transparent window to video | E (CSS mask) |
| **Text Scramble Effect** | Matrix-style decoding on load / hover | JS interval |
| **Circular Text Path** | Text curving along spinning circle | SVG `textPath` |
| **Gradient Stroke Animation** | Outlined text with running gradient | CSS `background-clip: text` |
| **Kinetic Typography Grid** | Letters dodging the cursor | JS + CSS |

### Micro-Interactions & Effects
| Pattern | Description | Notes |
|---------|-------------|-------|
| **Particle Explosion Button** | CTA shatters into particles on success | Canvas |
| **Liquid Pull-to-Refresh** | Reload indicator like detaching droplets | SVG filter |
| **Skeleton Shimmer** | Shifting light reflection across placeholders | CSS `@keyframes` |
| **Directional Hover-Aware Button** | Fill enters from cursor's exact side | JS `pointer` events |
| **Ripple Click Effect** | Wave from click coordinates | CSS `::after` + scale |
| **Animated SVG Line Drawing** | Vectors drawing themselves in real time | CSS `stroke-dashoffset` |
| **Mesh Gradient Background** | Organic lava-lamp blobs | CSS `@keyframes` + `blur()` |
| **Lens Blur Depth** | Background UI blurred to focus foreground action | `backdrop-filter: blur()` |

### Animation Library Choice
| Library | Use for | Notes |
|---------|---------|-------|
| **Motion** (`motion/react`) | UI / Bento / state-change motion | Default for reveals, layout transitions, micro-interactions |
...
| **Three.js / WebGL** | Canvas backgrounds and 3D scenes | Same isolation rule; lazy-load for bundle size |
