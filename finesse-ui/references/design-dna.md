...
**Sans font choice (discouraged as default):** `Inter` is the reflex default. Pick `Geist`, `Outfit`, `Cabinet Grotesk`, `Satoshi`, or `PP Neue Montreal` first. Inter is acceptable when the user explicitly asks for a neutral / Linear-style feel, or for public-sector / accessibility-first sites where familiarity is a design goal.

**Serif discipline (very discouraged as default):** Serif is **not** the default for "creative / premium / editorial" briefs. The agent's reflex that "creative brief = serif" is the single most-tested AI tell. Serif is only acceptable when ONE of these is true:
- The brand brief literally names a serif font, OR
- The aesthetic is genuinely editorial / luxury / publication / manuscript / heritage / vintage AND you can articulate why this specific serif fits this specific brand.

For everything else (creative agency, design studio, premium consumer, portfolio, lifestyle), **default sans-serif display** (Geist Display, Cabinet Grotesk Display, PP Neue Montreal, GT Walsheim, Inter Display, Söhne Breit). Sans display fonts are not "boring" — they are the default for the same reason black is the default in fashion.
...
**Serif pool (rotate, don't reuse):** If a serif is genuinely justified (rare), pick from this pool. Never reuse the same serif across consecutive projects:
...
**Italic descender clearance (mandatory):** When italic is used in display type and the word contains a descender letter (`y g j p q`), `leading-[1]` or `line-height: 1` will clip the descender. Use `leading-[1.1]` minimum and add `pb-1` or `mb-1` reserve on the wrapping element. Audit every italic word in display headlines before shipping.
...
|---------|-----------------|-------|
| Inter + JetBrains Mono | tech, fintech, science | the workhorse; mono carries labels/metrics. **Note:** Inter is over-defaulted — use it with a stated reason. |
| Geist + Geist Mono | modern SaaS, indie dev, AI | Geist is Vercel's font; a strong, less-tired alternative to Inter. |
| Satoshi + JetBrains Mono | contemporary brand, startup, agency | Satoshi has a distinctive geometric character while staying neutral. |
| Cabinet Grotesk + Inter Tight | editorial-tech, brand-forward SaaS | Grotesk display with a tightened sans body; clean and confident. |
| GT America + IBM Plex Mono | enterprise, B2B, professional | Mature, no-nonsense pairing; works for trust-heavy industries. |
...
**Premium-consumer palette rotation (mandatory):** If the previous premium-consumer project (cookware, wellness, artisan, luxury, DTC home goods) used the beige+brass+espresso family, the next one MUST use a different family. The beige+brass+espresso palette is the single most-recurring AI tell for premium-consumer briefs. Default alternatives that are NOT the warm-craft reflex: Cold Luxury (silver-grey + chrome + smoke), Forest (deep green + bone + amber), Black and Tan (true off-black + warm tan, no beige), Cobalt + Cream (saturated blue on one neutral, no brass), Terracotta + Slate (warm rust on cool grey), Olive + Brick + Paper, or pure monochrome + one saturated pop. **Override:** the beige+brass+espresso palette is acceptable ONLY when the brand brief explicitly names those colors, or when the brand identity is genuinely vintage / artisan / warm-craft AND you can articulate why this specific palette fits this specific brand.
