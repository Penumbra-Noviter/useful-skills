...
> **Route check — are you in the right file?** This file is for pages you **read**: dashboards, analytics, monitoring, data tables. If the page's primary action **commits** something (保存 / 发布 / 提交 / 配置 / 上线) — a merchant publishing, an admin configuring, a long form producing a real thing — it's an **operate** page and belongs in `workflow-ui.md` (which inherits everything here plus the workflow shell). If the page's spine is a **conversation or a live agent run** (AI chat / assistant / agent surfaces — the loop is prompt → watch it work → review/approve), it's an **assistant** page and belongs in `ai-native-ui.md` (which inherits everything here plus the assistant shell). A settings page wearing a dashboard shell is a route failure, not a style choice. Re-route before laying the shell; see SKILL.md §0.F.
