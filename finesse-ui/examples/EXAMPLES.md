...
> **Slop-cleanliness:** the corpus is blacklist-clean as of v0.17.0 — `detect.mjs` reports no em-dash, pure `#fff`/`#000`, Inter-usage, fake-precise, or banned-token hits. The only expected findings are the **deliberate soul elements** the personas are built on: chromatic glow on the dark pages (acru / buildly / inkline / signal / stakent), one marquee each (aether / nova / signal / studio), and pulse loops (aether / signal / studio). Finesse allows exactly one marquee per page, and these glows are the personas' signatures, not default slop — treat them as intentional. Any **other** finding on the corpus is a real defect: fix it.
