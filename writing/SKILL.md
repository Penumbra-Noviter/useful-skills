---
name: writing
description: 写作编排层：一个入口把长篇写作意图路由到 writing 三件套的正确阶段——writing-fragments（explore，访谈挖素材）/ writing-shape（exploit，线性成文）/ writing-beats（exploit，叙事成文）。通过检查已有写作产物文件判定当前阶段并自动衔接，替代用户手动串联三件套。Use when the user wants to write an article, essay, long post, story, or long-form prose; turn notes, material, or a transcript into a piece of writing; continue or reshape an already-started piece; or doesn't know how to start. 触发词：写文章、写一篇、写长帖、把素材写成文、这篇怎么写、帮我写个东西、write an article/essay/story/long post、turn this into prose、write this up. 不接管：短回复、评论、commit message、一句话文案——那不属于三件套的成文工作。
---
...
- [ ] 未越界：没有新增第四阶段，没有在编排层重述 grounding/append 等子 skill 既有机制
- [ ] 冲突先问：`article.md` 已存在且是新一篇时，先确认覆盖/另起名，没有静默覆盖

## 边界

- 不重写三件套的写作机制、文件格式、改写纪律；不把它们的规则复制到本文件。
- 不替三件套发明第四阶段（如"发布""排版"）——用户要求发布/排版时说明不在编排层内，按需走 open-kimi-ppt 等其他 skill。
- 唯一职责：意图 → 阶段 → 子 skill → 文件路径。路到了，执行就交给子 skill 的 SKILL.md。